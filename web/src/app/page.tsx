...
// ── SVG icon shapes (static — no locale needed) ───────────────────────────────
...
// Stable keys for React reconciliation — language-independent identifiers
const FEATURE_KEYS = ["regions", "skill", "tiers"] as const;
const FEATURE_ICONS = [MapIcon, ChartIcon, CheckIcon];

// Stat values are static numbers; only the labels are localized
const STAT_VALUES = ["13", "2", "27 yr", "HSS"] as const;
...
export default function LandingPage() {
  ...
  const features = FEATURE_KEYS.map((key, i) => ({
    ...
    title: [t.landing.feature1Title, t.landing.feature2Title, t.landing.feature3Title][i],
    body:  [t.landing.feature1Body,  t.landing.feature2Body,  t.landing.feature3Body][i],
  }));

  const statLabels = [
    t.landing.statRegions,
    t.landing.statSeasons,
    t.landing.statWindow,
    t.landing.statMetric,
  ];
    ...
    <div className="min-h-screen flex flex-col overflow-x-hidden" style={{ background: "var(--background)" }}>
      ...
      >
        ...
        <nav className="flex items-center gap-5 text-sm" style={{ color: "var(--text-muted)" }}>
          ...
          {/* Language toggle — same Zustand store as TopNav and all dashboard pages */}
          <button
            type="button"
            onClick={() => setLanguage(language === "en" ? "am" : "en")}
            aria-label={t.topNav.toggleLanguage}
            className="rounded-md px-2.5 py-1 text-xs font-semibold border transition-colors duration-150 hover:brightness-125"
            style={{
      ...
      <main className="relative z-10 flex flex-col items-center justify-center flex-1 px-6 pt-24 pb-16 text-center">
        ...
        {/* Brand name block — "Azmera" is the product name; "አዝመራ" is its Amharic script */}
        <h1
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 mb-20">
          ...
          >
            {t.landing.ctaExploreValidation}
            <svg
              viewBox="0 0 16 16"
              fill="none"
              stroke="currentColor"
